...
var planetUniform = {
  ...
  customColor: { type: "i", value: new THREE.Vector4(0.48, 0.89, 0.9, 1) },
};
...
function init() {
  ...
  renderer.setSize(
    Math.round(window.innerWidth / resolution),
    Math.round(window.innerHeight / resolution)
  );
  ...
  renderer.domElement.style.width =
    Math.round(renderer.domElement.width * resolution) + "px";
  renderer.domElement.style.height =
    Math.round(renderer.domElement.height * resolution) + "px";
  ...
  ShaderLoader(
    "js/Shaders/Planet/Planet.vs.glsl",
    "js/Shaders/Planet/Planet.fs.glsl",
    setUpPlanet,
    true
  );
  ...
  ShaderLoader(
    "js/Shaders/Sky/Sky.vs.glsl",
    "js/Shaders/Sky/Sky.fs.glsl",
    setUpSky,
    true
  );
...
function onWindowResize() {
  ...
  if (onMobile) {
    ...
    renderer.domElement.style.width =
      Math.round((renderer.domElement.width / 2) * resolution) + "px";
    renderer.domElement.style.height =
      Math.round((renderer.domElement.height / 2) * resolution) + "px";
  } else {
    console.log("OnPc");
    renderer.setPixelRatio(window.devicePixelRatio);
    composer.setSize(
...
function MoonsUpdate(delta) {
  ...
  if (moonList !== undefined) {
    ...
    for (var i = 0; i < moonList.length; i++) {
      moonList[i].text.updatePosition(
        moonList[i].moonSize,
        -moonList[i].text.element.clientWidth / 2,
        55
      );
      orbit(
        moonList[i],
        moonList[i].moonObject,
        new THREE.Vector3(0, 0, 0),
        clock.getElapsedTime() * moonList[i].orbitSpeedMult,
        1000,
        delta / 12
      );
...
function input() {
  if (planet !== undefined) MouseInPlanet(planet, planetSize);

  if (moonList !== undefined) {
    for (var i = 0; i < moonList.length; i++) {
      MouseInMoon(
        moonList[i].moonObject,
        moonList[i].moonSize + 5,
        moonList[i]
      );
    }
  }
    ...
    for (var i = 0; i < ringsList.length; i++) {
      if (!ringsList[i].isFlat) {
        ...
        if (ShaderLoadList.ring.vertex == undefined) {
          ShaderLoader(
            "js/Shaders/Ring/Ring.vs.glsl",
            "js/Shaders/Ring/Ring.fs.glsl",
            SetUpFlatBelt,
            { data: ringsList[i], Ringcolors: AstoColorPalleteGrab }
          );
        } else {
          CreateFlatBelt(
            { data: ringsList[i], Ringcolors: AstoColorPalleteGrab },
            ShaderLoadList.ring.vertex,
            ShaderLoadList.ring.fragment
          );
        }
      }
...
function InitializeRingsData(ringsList) {
  for (var i = 0; i < ringsList.length; i++) {
    ...
    if (roll >= 4) flat = false;
    else flat = true;

    if (i == 0) {
      orbitrangeInner = randomRange(
        (planetSize / 1000) * 1.1,
        (planetSize / 1000) * 1.2
      );
      orbitrangeOutter = randomRange(
        orbitrangeInner * 1.05,
        orbitrangeInner * 1.15
      );
    } else if (i >= 1) {
      orbitrangeInner = randomRange(
        ringsList[i - 1].a1,
        ringsList[i - 1].a1 * 1.15
      );
      orbitrangeOutter = randomRange(
        orbitrangeInner * 1.05,
        orbitrangeInner * 1.15
...
function InitializeMoonData(moonList, vertex_text, fragment_text) {
  for (var i = 0; i < moonList.length; i++) {
    ...
    moonList[i] = {
      ...
      w1: 318.0634,
      w2: 0.1643573223,
      a1: randomRange(planetSize / 1000 + 0.02, 0.32),
      a2: 0,
      e1: 0,
      e2: 0,
      M1: 115.3654,
...
function TargetUI() {
  targetBox.bottomR = generateImageUi(
    new THREE.Vector3(0, 0, 0),
    "110px",
    -1000,
    "img/Icons/Corner_Bottom_Right.png",
    "UI-Target",
    "Target-Ui-Label"
  );
  targetBox.bottomR.updatePosition(1, 500, 500);

  targetBox.bottomL = generateImageUi(
    new THREE.Vector3(0, 0, 0),
    "110px",
    -1000,
    "img/Icons/Corner_Bottom_Left.png",
    "UI-Target",
    "Target"
  );
  targetBox.bottomL.updatePosition(1, 500, 500);

  targetBox.topR = generateImageUi(
    new THREE.Vector3(0, 0, 0),
    "110px",
    -1000,
    "img/Icons/Corner_Top_Right.png",
    "UI-Target",
    "Target"
  );
  targetBox.topR.updatePosition(1, 500, 500);

  targetBox.topL = generateImageUi(
    new THREE.Vector3(0, 0, 0),
    "110px",
    -1000,
    "img/Icons/Corner_Top_Left.png",
    "UI-Target",
    "Target"
  );
...
function createAtmos(colors, vertex_text, fragment_text) {
  ...
  atmouniforms.fresnelExp.value = randomRange(0.1, 1.99);
  ...
  atmouniforms.atmoThickness.value = randomRange(0.0, 3.0);
  ...
  atmo = new THREE.Mesh(
    new THREE.IcosahedronGeometry(planetSize * randomRange(1.01, 1.05), 4),
    atmoMaterial
  );
...
function createPlanet(start, vertex_text, fragment_text) {
  if (planet !== undefined) {
    ...
    //TargetUI();
    refreshPlanetButton = generateButtonUi(
      new THREE.Vector3(0, 0, 0),
      "110px",
      -1000,
    ...
    showPlanetButton = generateButtonUi(
      new THREE.Vector3(0, 0, 0),
      "110px",
      -1000,
      "",
  ...
  planet = new THREE.Mesh(cube, PlanetMaterial);
  planet.castShadow = true; //default is false
  planet.receiveShadow = true; //default

  if (planetData.url == "") {
    planet = new THREE.Mesh(ico, PlanetMaterial);
    planet.castShadow = true; //default is false
    planet.receiveShadow = true; //default
  ...
  if (ShaderLoadList.atmo.vertex == undefined) {
    ShaderLoader(
      "js/Shaders/Atmo/AtmoShader.vs.glsl",
      "js/Shaders/Atmo/AtmoShader.fs.glsl",
      setUpAtmosphere,
      planetData.colors
    );
  } else {
    createAtmos(
      planetData.colors,
      ShaderLoadList.atmo.vertex,
      ShaderLoadList.atmo.fragment
    );
  }

  for (var i = 0; i < moonList.length; i++) {
    moonList[i].moonOrbit = DrawOrbit(
      moonList[i],
      new THREE.Vector3(0, 0, 0),
      clock.getElapsedTime(),
      1000,
    ...
    if (moonList[i].moonOrbit != 0) MainScene.add(moonList[i].moonOrbit);

    moonList[i].text = generateName(
      moonList[i].moonObject,
      "35px",
      -1000,
      false
    ); // generateName(planet, "35px", -1000);
  }
  ...
  if (roll >= 5) {
    ...
    if (ShaderLoadList.asto.vertex == undefined) {
      ShaderLoader(
        "js/Shaders/Asto/Asto.vs.glsl",
        "js/Shaders/Asto/Asto.fs.glsl",
        setUpRings,
        planetData.colors
      );
    } else {
      setUpRings(
        planetData.colors,
        ShaderLoadList.asto.vertex,
        ShaderLoadList.asto.fragment
      );
    }
  } else {
    hasRings = false;
  ...
  planetTextInfo = generateName(
    planet,
    1,
    -1000,
    true,
    planetData.colors,
    planetData.regionsInfo
  );
...
function createPlantiodData(
  ...
  var planetInfo = new MapGenerator(
    octaves,
    persistance,
    lacunarity,
    seed,
...
function generateButtonUi(parent, fontsize, left, url, classname, id) {
  ...
  if (url != "") newUI.setHTML("<img src=" + url + ">");
  else {
  ...
  if (id == "Refresh") ul = document.getElementById("UI-Tools-List");
  else ul = document.getElementById("UI-Tools-List");
  ...
  if (document.getElementById("Refresh") != null) {
    document.getElementById("Refresh").onclick = function () {
      if (ShaderLoadList.planet.vertex == undefined) {
        ...
        createPlanet(
          false,
          ShaderLoadList.planet.vertex,
          ShaderLoadList.planet.fragment
        );
      }
    };
  }
  ...
  if (roll > 5 && !isInfo) {
    ...
    newText.setHTML(
      name +
        ...
        CE +
        "<br>" +
        buildElements(colorpallette, regions) +
        "<br>" +
        "" +
        "<br>" +
        bio
      ...
      fragment_loader.load(fragment_url, function (fragment_text) {
        ...
        shadow_loader.load("js/Shaders/Shadow.glsl", function (shadow_text) {
          ...
          dither_loader.load("js/Shaders/Dither.glsl", function (dither_text) {
            onLoad(
              Custom,
              textParse(vertex_text, shadow_text, dither_text),
              textParse(fragment_text, shadow_text, dither_text)
            );
          });
        });
      });
