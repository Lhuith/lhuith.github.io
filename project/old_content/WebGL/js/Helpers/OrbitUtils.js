...
function returnOrbitionPosition(ringData, i, inner, centre, replaceW) {
  ...
  } else a = ringData["a1"] + ringData["a2"] * i;
...
function DrawOrbit(planet, centre, currTimeD, auScale, color, i, max) {
  ...
  moonorbit = new THREE.Line(
    ...
    new THREE.MeshBasicMaterial({
      color: colors[randomRangeRound(0, colors.length - 1)].hex,
      transparent: true,
      opacity: 1.0,
    })
  );
...
function CreateRockyBelt(
  ...
  for (var i = 0; i < segmentCount; i++) {
    var texture =
      astroidSprites[Math.round(randomRange(0, astroidSprites.length - 1))];
    ...
    var uniform = {
      ...
      color: {
        type: "vf3",
        value: colors[randomRangeRound(0, colors.length - 1)].RGB,
      },
